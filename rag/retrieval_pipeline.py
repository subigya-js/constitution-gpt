...
def create_structured_context(docs):
    ...
    for (part, article, article_title), doc_list in grouped.items():
        ...
        sorted_docs = sorted(doc_list, key=lambda d: (
...
def extract_key_terms(query):
    ...
    stop_words = {'how', 'what', 'when', 'where', 'who', 'why', 'is', 'are', 'the', 'a', 'an', 'in', 'of', 'to', 'for', 'and', 'or', 'nepal', 'constitution'}
...
def expand_query(original_query):
    ...
    if "fundamental rights" in query_lower or ("rights" in query_lower and "citizen" in query_lower):
        expansions.extend([
    ...
    if "duties" in query_lower and "citizen" in query_lower:
        expansions.extend([
            "Article 48 duties citizens",
            "responsibilities citizens Nepal"
        ])
    
    if "president" in query_lower and ("elect" in query_lower or "appoint" in query_lower):
        expansions.extend([
...
def retrieve_and_answer(query, verbose=True):
    ...
    for doc in all_docs[:15]:  # Check top 15 docs
        ...
        if article:
            ...
            if term_count >= 2 or len(query_key_terms) <= 1:  # At least 2 terms or single-term query
    ...
    if key_articles_found:
        ...
        for i, metadata in enumerate(all_db_docs['metadatas']):
            if metadata and metadata.get('article') in key_articles_found:
                ...
                doc = Document(
                    page_content=all_db_docs['documents'][i],
                    metadata=metadata
                )
    ...
    priority_docs.sort(key=lambda x: (-x[0], x[1].metadata.get('article', 'ZZZ'), x[1].metadata.get('subarticle', 'ZZZ')))
